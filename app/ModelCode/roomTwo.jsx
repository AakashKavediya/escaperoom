...
function WebsiteScreen() {
  ...
  return (
    <Html
      transform
      position={[3.9, 3.2, 0]} // Exact position to match the monitor in the model
      rotation={[0, 287.5, 0]}
      scale={scale}
      distanceFactor={1}
      occlude={false}
...
const FirstPersonControls = ({ onPositionUpdate }) => {
  ...
  const boundary = {
    points: [
      new THREE.Vector3(4.22, 3.00, 4.61),   // Point 1
      new THREE.Vector3(-0.57, 3.00, 4.15),  // Point 2
      new THREE.Vector3(-0.33, 3.00, -1.73), // Point 3
      new THREE.Vector3(3.62, 3.00, -1.15)   // Point 4
    ],
    y: 3.00 // Fixed Y position
  };
  
  // Camera state (first person position) - starting at [0, 3, 0]
  const [cameraPosition, setCameraPosition] = useState([2, 3, -0.15]);
  
  // Mouse look controls
  const [isMouseLooking, setIsMouseLooking] = useState(false);
  const previousMousePosition = useRef({ x: 0, y: 0 });
  
  // Movement parameters - decreased by 40%
  const moveSpeed = 0.024; // Reduced from 0.04 by 40%
  const damping = 0.8;
  const mouseSensitivity = 0.002;

  // Refs for useFrame
  const cameraPosRef = useRef(new THREE.Vector3(2, 3, -0.15));
  ...
  const clampPosition = useCallback((position) => {
    ...
    if (!isPointInQuadrilateral(clampedPosition, boundary.points)) {
      // If outside, find the closest point on the boundary
      // For simplicity, we'll use a simple projection to the nearest edge
      // This is a simplified approach - you might want more sophisticated collision detection
      
      // Keep the previous valid position
  ...
  useFrame(() => {
    ...
    // Apply movement speed (40% reduced)
